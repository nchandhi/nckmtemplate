...
curl --output "02_create_cu_template.py" ${baseUrl}"infra/scripts/index_scripts/02_create_cu_template.py"
curl --output "03_cu_text_processing.py" ${baseUrl}"infra/scripts/index_scripts/03_cu_text_processing.py"
# RUN apt-get update
...
sed -i "s/kv_to-be-replaced/${keyvaultName}/g" "02_create_cu_template.py"
sed -i "s/kv_to-be-replaced/${keyvaultName}/g" "03_cu_text_processing.py"

pip install -r requirements.txt

python 01_create_search_index.py
python 02_create_cu_template.py
python 03_cu_text_processing.py
